...
use ztp_rust::{configuration, email_client, startup::run, telemetry::{get_subscriber, init_sub}};
use sqlx::PgPool;
...
async fn main() -> Result<(), std::io::Error> {
    ...
    let sender = config.email.sender().expect("Failed to parse sender email from config");
